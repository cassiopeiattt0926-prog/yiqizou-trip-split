...
has(sw, "haoyouji-v28", "缓存版本");
assert.ok(!`${html}\n${sw}\n${js}`.includes("v=27"), "仍残留旧缓存版本 v27");
...
has(html, "rate-policy.js?v=28", "独立汇率策略模块");
...
assert.ok(html.indexOf('id="share"') < html.indexOf('应收应付明细'), "分享结算单必须在应收应付明细上方");
...
has(js, "drawQrFooter", "分享图片二维码");
has(js, "扫描二维码，开始使用好友记", "二维码引导文案");
has(js, "personalShareFile", "个人消费图片清单");
has(js, "生成${escapeHtml(p.name)}的个人消费清单", "个人消费清单按钮");
has(js, 'details.open ? "收起" : "查看明细"', "个人统计展开收起文案");
has(js, "setupStatShareSwipeRows", "个人统计左滑生成清单");
has(css, ".stat-share-action", "个人统计绿色生成按钮");
has(js, 'focusForTyping($("#tripName"))', "新建旅行自动聚焦");
has(js, 'focusForTyping($("#amount"))', "记一笔自动聚焦");
has(js, "focusNextTripInput", "新建旅行回车跳转");
